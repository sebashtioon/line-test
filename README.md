# line test
please standby

for hackclub horizons :)

> disclaimer: copilot *was* used once, but only for resolving merge conflicts in godot scene files that would have taken me hours otherwise for no reason whatsoever


---

school pc lapse code:
soldier neglect tomato nasty flip now small empty ring limb they jacket

---

storyboard


## game
- player picks up phone, voice tells them that this is a dream and everything they see isnt real
- voice tells them their gonna take a sequence of tests. nothing much. wont take long
- 




## sequence (DREAM SEQUENCE - SVDDEN DEATH REFERENCE????? WTF)

1. fade in -> player is some distance from the phone but can still see it on the table to some extent
1.5. light flickers a bit
2. phone is ringing
3. player walks over to ringing phone
4. phone is still ringing
5. player intrigued to pick up phone
6. player picks up phone

- some static noises play from the phone to set the creepy tone
- phone audio plays mostly in the ear that the phone being held in (probably right ear) for extra immersion
- a deep voice with bitcrusher and low quality starts talking, with a mini dialogue box to go with it


voice dialogue

1. "hello. this is a line test. please standby"

- some glitchy static noises and also some dial tones (maybe like 5-10 seconds)

2. "what you are seeing isnt real"
3. "
